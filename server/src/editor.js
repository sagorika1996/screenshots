...
exports.Editor = class Editor extends React.Component {
    ...
    return <div>
      <div className="editor-header default-color-scheme">
        <div className="shot-main-actions">
          <a className={`button pen-button ${this.state.pen}`} id="pen" onClick={this.onClickPen.bind(this)} title="pen"></a>
          <a className={`button highlight-button ${this.state.highlighter}`} id="highlight" onClick={this.onClickHighlight.bind(this)} title="highlighter"></a>
        </div>
  ...
  onClickHighlight() {
    if (this.state.tool != 'highlighter') {
      this.setState({tool: 'highlighter', highlighter: 'active', pen: 'inactive'});
      sendEvent("highlighter-select", "annotation-toolbar");
    } else {
      this.setState({tool: 'none', highlighter: 'inactive'});
  ...
  onClickPen() {
    if (this.state.tool != 'pen') {
      this.setState({tool: 'pen', pen: 'active', highlighter: 'inactive'});
      sendEvent("pen-select", "annotation-toolbar");
    } else {
      this.setState({tool: 'none', pen: 'inactive'});
